...
const Favorite = () => {
  ...
  const allRadios = useSelector((state) => state.radio.radios);
  const radios = allRadios.filter((radio) => radio.favorite);
  const allSearchRadios = useSelector(
    (state) => state.radio.filteredSearchRadios
  );
  const searchRadios = allSearchRadios.filter((radio) => radio.favorite);
